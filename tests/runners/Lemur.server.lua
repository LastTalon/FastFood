...
local Source = { -- This can potentially be loaded from a project.json
	...
	{ "tests/integration/", "ReplicatedStorage.Tests.Integration" },
	{ "tests/runners/", "ReplicatedStorage.Tests.Runners" },
	{ "tests/unit/", "ReplicatedStorage.Tests.Unit" },
	{ "modules/testez/", "ReplicatedStorage.Modules.TestEZ" },
}
...
for _, pair in ipairs(Source) do
	...
	if #tokens == 1 then
		...
		for i = 2, #tokens - 1 do
			local found = container:FindFirstChild(tokens[i])
			if not found then
				found = Habitat.Instance.new("Folder")
				found.Name = tokens[i]
...
local Tests = Habitat:require(
	ReplicatedStorage
		:WaitForChild("Tests")
		:WaitForChild("Runners")
		:WaitForChild("Tests")
)
